...
class Scheduling{
  ...
  public static int Timeslice;  //타임슬라이스
  ...
  public void inputProcess(){ //프로세스 정보 입력
    ...
    for(int i=0;i<3;i++){     //3개의 프로세스 생성
      System.out.print("\n");
      System.out.printf("Enter Arrival Sequence of Process(1 or 2 or 3) %d: ",i+1);          //프로세스 순서 입력
      int as = sc.nextInt();
      System.out.printf("Enter Arrival Time of Process (one process must be 0) %d: ",i+1);   //프로세스 도착 시간 입력
      int at = sc.nextInt();
      System.out.printf("Enter Processing Time of Process %d: ",i+1);                        //프로세스 작업 시간 입력
      int pt = sc.nextInt();
      System.out.print("\n");
...
//---------------------------------------------------------------------------
  public void inputTimeslice(){         //타임 슬라이스 입력
    Scanner sc=new Scanner(System.in);
...
//---------------------------------------------------------------------------
  public void inputQueue(){ //시작할 때 프로세스를 넣음
    ...
    sd.inputTimeslice();  //타임슬라이스 입력 받음
  ...
  public void startScheduling(){  //스케줄링 시작
    ...
    while(queue.rear!=queue.front){     //큐가 비어있지 않으면
      if(queue.circular_Queue[(queue.front+1)%4].rt<=Timeslice){  //(queue.front+1)%4는 front 다음 번을 가리킴, 큐에 맨 앞에 들어있는 프로세스를 가리큼
        //프로세스의 남은 시간이 타임슬라이스보다 작거나 같은 경우, 프로세스가 종료될 때에는 무조건 이 영역에 들어오게 된다.
        if(queue.circular_Queue[(queue.front+1)%4].at<=Time){ //프로세스의 도착 시간이 현재 시간보다 큰 경우, 프로세스가 도착하지 않았다고 가정하고 실행하지 않는다.
          queue.circular_Queue[(queue.front+1)%4].it += queue.circular_Queue[(queue.front+1)%4].rt; //프로세스가 진행된 시간에 이번 프로세스가 진행될 시간(여기서는 이번에 진행될 프로세스의 남은 시간)을 더한다.
          if((queue.front+1)%4!=queue.rear){  //만약에 큐에 이번에 실행하는 프로세스 이외에 다른 프로세스가 남아있다면
            for(int k=2;k<=3;k++){  //다른 프로세스가 남아봤자 1개 혹은 2개일거임
              if((queue.rear+1)%4 != (queue.front+k)%4){  //남은 프로세스의 개수 체크
                queue.circular_Queue[(queue.front+k)%4].wt+=queue.circular_Queue[(queue.front+1)%4].rt; //남은 프로세스의 대기 시간에 이번에 실행된 프로세스의 실행 시간(여기서는 이번에 실행된 프로세스의 남은 시간)만큼 추가
              }
            }
          }

          Time+=queue.circular_Queue[(queue.front+1)%4].rt; //전체 시간에 이번에 실행된 프로세스의 실행 시간(여기서는 이번에 실행된 프로세스의 남은 시간)만큼 추가
      ...
      /////////////////////////////////////////////////////////////////////////////////////////////////////////
      else{   //프로세스의 남은 시간이 타임슬라이스보다 큰 경우, else에서 실행 된 경우 이번에 프로세스가 끝나지 않고 다시 대기 큐의 맨 뒤로 들어가게 된다.
          if(queue.circular_Queue[(queue.front+1)%4].at<=Time){ //프로세스의 도착 시간이 현재 시간보다 큰 경우, 프로세스가 도착하지 않았다고 가정하고 실행하지 않는다.
            queue.circular_Queue[(queue.front+1)%4].it += Timeslice; //프로세스가 진행된 시간에 이번 프로세스가 진행될 시간(여기서는 타임슬라이스)을 더한다.
            System.out.printf("process%d!\n",queue.circular_Queue[(queue.front+1)%4].processname);  //프로세스 (이름)이 이번에 실행되었음을 출력

            if((queue.front+1)%4!=queue.rear){  //만약에 큐에 이번에 실행하는 프로세스 이외에 다른 프로세스가 남아있다면
              for(int k=2;k<=3;k++){  //다른 프로세스가 남아봤자 1개 혹은 2개일거임
                if((queue.rear+1)%4 != (queue.front+k)%4){  //남은 프로세스의 개수 체크
                  queue.circular_Queue[(queue.front+k)%4].wt+=Timeslice;//남은 프로세스의 대기 시간에 이번에 실행된 프로세스의 실행 시간(여기서는 타임슬라이스)만큼 추가
                }
              }
            }
            Time+=Timeslice; //전체 시간에 이번에 실행된 프로세스의 실행 시간(여기서는 타임슬라이스)만큼 추가
            queue.circular_Queue[(queue.front+1)%4].rt = queue.circular_Queue[(queue.front+1)%4].pt - queue.circular_Queue[(queue.front+1)%4].it; //이번에 실행된 프로세스의 남은 시간=전체 작업 시간 - 현재까지 진행된 시간
            ...
            queue.EnQueue(queue.DeQueue());//프로세스가 끝나지 않았으므로 포인터를 옮기고, 이번에 실행된 프로세스를 대기 큐의 맨 뒤에 넣음
          }
          else{
            queue.EnQueue(queue.DeQueue());//프로세스가 아직 도착하지 않았으므로 다시 맨 뒤로 옮김
          }
